...
# 定义人脸标签和初始化标签对应的人物名称
id = 0
names = ['undefined' for i in range(100)]
music1Path = r'./test.mp3'
music2Path= r'./warning.mp3'
music3Path=r'./right.mp3'
musicFlag = False
...
GPIO.setup(6,GPIO.OUT,initial=True)
...
faceCascade2 = cv2.CascadeClassifier('/home/pi/haarcascade_frontalface_default.xml')
faceCascade3 = cv2.CascadeClassifier('/home/pi/haarcascade_frontalface_default.xml')
...
class Ui_Menu(QWidget):
    def __init__(self):
        ...
        # 创建label并设置文本内容
        self.label = QLabel('居家隔离防疫监控系统', self)
        # 创建普通用户和管理员按键
        self.btn_ordinary = QPushButton('人员监控', self)
        self.btn_admin = QPushButton('人员录入', self)
...
class Ui_logon(QWidget):
    ...
    def __init__(self):
        ...
        self.loginHint=QLabel('安全认证',self)
    ...
    def init_ui(self):
        ...
        self.loginHint.setGeometry(380,10,400,100)
        ...
        self.loginHint.setStyleSheet(
            "QLabel{color:rgb(0,0,0,255);"  # 字体颜色为黑色
            "font-size:60px;font-weight:bold;"  # 大小为30 
            "font-family:Roman times;}")  # Roman times字体
    ...
    def slot_btn_logon(self):
        # 判断账号和密码是否输入正确
        if self.ID_num == "" and self.key_num == "":
...
class Ui_manager_face(QWidget):
    def __init__(self):
        ...
        self.lab2=QLabel(self.groupBox)
        self.Edit_ID = QLineEdit(self.groupBox)
        self.edit2=QLineEdit(self.groupBox)
    ...
    def init_ui(self):
        ...
        # 对groupBox进行设置
        self.groupBox.setTitle("人员录入管理")
        GPIO.output(20,GPIO.HIGH)
        GPIO.output(16,GPIO.HIGH)
        GPIO.output(12,GPIO.LOW)
        self.groupBox.setGeometry(990, 40, 281, 310)
        ...
        # 设置groupBox里面的控件
        self.lab_ID.setText("编号")
        self.lab_ID.setGeometry(10, 40, 61, 41)
        self.lab_ID.setStyleSheet("font: 15pt;")

        self.Edit_ID.setGeometry(60, 40, 200, 41)
        self.Edit_ID.setStyleSheet("font: 15pt;")
        
        self.lab2.setText("名称")
        self.lab2.setGeometry(10,100,61,41)
        self.lab2.setStyleSheet("font: 15pt;")
        
        self.edit2.setGeometry(60,100,200,41)
        self.edit2.setStyleSheet("font: 15pt;")

        self.btn_enter.setText("开始录入")
        self.btn_enter.setGeometry(10, 160, 250, 40)
        self.btn_enter.setStyleSheet("font: 75 16pt;")

        self.progressBar.setGeometry(10, 260, 250, 31)
        # self.progressBar.setProperty("value", 0)

        self.btn_back.setText("返回")
        self.btn_back.setGeometry(10, 210, 250, 40)
    ...
    def slot_btn_enter(self):
        self.count = 0
        self.step = 0
        names[int(self.Edit_ID.text())]=self.edit2.text()
        # 创建线程并开启
    ...
    def thread_pic(self):
        ...
        # 将训练好的数据保存到指定文件夹中
        self.recognizer.save(self.triningfile + "/trainer.yml")
...
class Ui_face_reco(QWidget):
    def __init__(self):
        ...
        self.localNotice=QLabel(self)
        self.grpbox=QtWidgets.QGroupBox(self)
        self.remoteWarn=QLabel(self.grpbox)
        self.remoteTips=QLabel(self.grpbox)
        self.remoteMsg=QLabel(self.grpbox)
        self.sndTips=QLabel(self.grpbox)
        self.remoteSnd=QLineEdit(self.grpbox)
    ...
    def camera_init(self):
        ...
        # 读取训练好的数据
        self.recognizer.load('./Face_training//trainer.yml')
        self.cascadePath = "/home/pi/haarcascade_frontalface_default.xml"
    ...
    def init_ui(self):
        ...
        self.lab_body.setGeometry(1000, 20, 350, 91)
        self.lab_ID.setGeometry(1000, 150, 130, 30)
        self.lab_ID_E.setGeometry(1150, 150, 130, 30)
        self.lab_T_F.setGeometry(1000, 90, 300, 81)
        self.localNotice.setGeometry(1000,180,350,81)
        
        self.grpbox.setTitle("远程通信")
        self.grpbox.setGeometry(1000, 285, 370, 370)
        self.grpbox.setStyleSheet(
            "QGroupBox {\n"
            "border-width:2px;\n"
            "border-style:solid;\n"
            "border-color:lightGray; \n"
        ...
        self.remoteWarn.setText("管理员未对您发出提醒！")
        self.remoteWarn.setGeometry(10, 40, 300, 41)
        self.remoteWarn.setStyleSheet("font: 18pt;")
        
        self.remoteTips.setText("管理员消息：")
        self.remoteTips.setGeometry(10, 90, 300, 41)
        self.remoteTips.setStyleSheet("font: 18pt;")
        
        self.remoteMsg.setText("暂无")
        self.remoteMsg.setGeometry(10, 155, 350, 71)
        self.remoteMsg.setStyleSheet("font: 18pt;")
        
        self.sndTips.setText("请在此输入您要发送消息：")
        self.sndTips.setGeometry(10, 245, 350, 41)
        self.sndTips.setStyleSheet("font: 18pt;")
        
        self.remoteSnd.setGeometry(10, 290, 350, 65)
        self.remoteSnd.setStyleSheet("font: 18pt;")
        ...
        self.lab_body.setStyleSheet("font: 75 20pt ; font-weight: bold;")
        self.lab_body.setText("正在监控人员来访情况")
        
        self.lab_ID.setStyleSheet("font: 75 16pt ;")
        self.lab_ID_E.setStyleSheet("font: 75 16pt ;")
        self.lab_T_F.setStyleSheet("font: 75 20pt; font-weight: bold;")
        
        self.lab_ID.setText("来访人员：")
        self.localNotice.setText("当前没有呼叫请求！")
        self.localNotice.setStyleSheet("font:75 30pt; font-weight: bold;")
        
        GPIO.output(20,GPIO.HIGH)
        GPIO.output(16,GPIO.HIGH)
        GPIO.output(12,GPIO.HIGH)
        
        self.thread = threading.Thread(target=self.thread_record)
        self.thread.start()
        
        self.thread2=threading.Thread(target=self.checkNotice)
        self.thread2.start()
        
        self.thread3=threading.Thread(target=self.checkRemote)
        self.thread3.start()
    ...
    def show_camera(self):
        ...
        for(x,y,w,h) in faces:
            ...
            if (confidence < 90):
                fileTemp=r'/var/www/html/faceOut.txt'
                with open(fileTemp,'a') as ft:
                    ft.write(datetime.datetime.now().strftime('%F %T')+' '+str(id)+' '+names[id]+'\n')
                id = names[id]
                confidence = "  {0}%".format(round(100 - confidence))
                # GPIO.output(25, GPIO.HIGH)   #GPIO25 输出3.3V
                self.lab_T_F.setText( "检测到授权接触人员。")
                self.lab_ID_E.setText(str(id))
                GPIO.output(20,GPIO.HIGH)
                GPIO.output(16,GPIO.LOW)
                GPIO.output(12,GPIO.LOW)
                ret, self.img33 = self.cap.read()
                self.img33 = cv2.flip(self.img33, -1)
                tmpNam=str(int(round(time.time()*1000)))+'.png'
                cv2.imwrite('/var/www/html/good/'+tmpNam,self.img33)
                fileTemp=r'/var/www/html/good.txt'
                with open(fileTemp,'a') as ft:
                  ft.write(tmpNam+'\n')
                
                p.ChangeDutyCycle(2.5)
                self.thread = threading.Thread(target=self.thread_music3)
                self.thread.start()
                time.sleep(0.2)   
                p.ChangeDutyCycle(0)   
            else:
                ...
                fileTemp=r'/var/www/html/faceOut.txt'
                with open(fileTemp,'a') as ft:
                    ft.write(datetime.datetime.now().strftime('%F %T')+' '+'-1'+' '+'unauthorized'+'\n')
                
                confidence = "  {0}%".format(round(100 - confidence))
                self.lab_T_F.setText( "检测到未授权接触人员！")
                self.lab_ID_E.setText("陌生访客")
                GPIO.output(20,GPIO.LOW)
                GPIO.output(16,GPIO.HIGH)
                GPIO.output(12,GPIO.LOW)
                ret, self.img33 = self.cap.read()
                self.img33 = cv2.flip(self.img33, -1)
                tmpNam=str(int(round(time.time()*1000)))+'.png'
                cv2.imwrite('/var/www/html/bad/'+tmpNam,self.img33)
                fileTemp=r'/var/www/html/bad.txt'
                with open(fileTemp,'a') as ft:
                    ft.write(tmpNam+'\n')
                p.ChangeDutyCycle(2.5 + 10 * 100 / 180)
                self.thread = threading.Thread(target=self.thread_music2)
                self.thread.start()
                time.sleep(0.2)   
                p.ChangeDutyCycle(0)
    ...
    def thread_music1(self):
        global musicFlag
        if(musicFlag == False):
            musicFlag=True
            os.system('mplayer %s' % music1Path)
            musicFlag=False
            
    def thread_music2(self):
        global musicFlag
        if(musicFlag == False):
            musicFlag=True
            os.system('mplayer %s' % music2Path)
            musicFlag=False
            
    def thread_music3(self):
        global musicFlag
        if(musicFlag == False):
            musicFlag=True
            os.system('mplayer %s' % music3Path)
            musicFlag=False
            
    def thread_record(self):
        while(True):
            os.system('python bme280.py')
            print('ddd')
            time.sleep(10)
            
    def checkNotice(self):
        while(True):
            noticeVal=GPIO.input(26)
            if(noticeVal==0):
                self.localNotice.setText("当前没有呼叫请求！")
            else:
                self.localNotice.setText("当前有人正在呼叫！")
            time.sleep(0.1)
    ...
    def checkRemote(self):
        while(True):
            ...
            rmtdta=rmtmsg.read()
            rmtspeak=rmtdta[2:]
            self.remoteMsg.setText(rmtspeak)
            fileTemp=r'/var/www/html/msg.txt'
            with open(fileTemp,'w') as ft:
                ft.write(self.remoteSnd.text())
            if(rmtdta[0]=='0'):
                GPIO.output(27,False)
                self.remoteWarn.setText("管理员未对您发出提醒！")
            else:
                GPIO.output(27,True)
                self.remoteWarn.setText("管理员正在对您发出提醒！")
            time.sleep(1)
